...
export class PageObject {
  ...
  async setUpSyrexPro({ autoApprove = false }: { autoApprove?: boolean } = {}) {
    await this.baseSetup();
    await this.goToSettingsTab();
    if (autoApprove) {
      await this.toggleAutoApprove();
    }
    await this.setUpSyrexProvider();
    await this.goToAppsTab();
  }
  ...
  async setUpSyrexProvider() {
    await this.page
      .locator("div")
      .filter({ hasText: /^SyrexNeeds Setup$/ })
      .nth(1)
      .click();
    await this.page.getByRole("textbox", { name: "Set Syrex API Key" }).click();
    await this.page
      .getByRole("textbox", { name: "Set Syrex API Key" })
      .fill("testSyrexkey");
  ...
  async snapshotMessages({
    ...
    if (replaceDumpPath) {
      // Update page so that "[[Syrex-dump-path=*]]" is replaced with a placeholder path
  ...
  async snapshotServerDump(
    ...
    const dumpPaths = dumpPathMatches
      .map((match) => {
        const pathMatch = match.match(/\[\[Syrex-dump-path=([^\]]+)\]\]/);
    ...
    ).replaceAll(/\[\[Syrex-dump-path=([^\]]+)\]\]/g, "[[Syrex-dump-path=*]]");
  ...
  getChatInput() {
    return this.page.locator(
      '[data-lexical-editor="true"][aria-placeholder="Ask Syrex to build..."]',
    );
  }
  ...
  async setUpAzure({ autoApprove = false }: { autoApprove?: boolean } = {}) {
    ...
    // so we don't need additional setup steps like setUpSyrexProvider
    await this.goToAppsTab();
  }
  ...
  getAppPath({ appName }: { appName: string }) {
    return path.join(this.userDataDir, "Syrex-apps", appName);
  }
...
export const test = base.extend<{
  ...
  po: [
    async ({ electronApp }, use) => {
      ...
      const po = new PageObject(electronApp, page, {
        userDataDir: (electronApp as any).$SyrexUserDataDir,
      });
      await use(po);
    },
    { auto: true },
  ...
  electronApp: [
    async ({ electronConfig }, use) => {
      ...
      process.env.Syrex_ENGINE_URL = "http://localhost:3500/engine/v1";
      process.env.Syrex_GATEWAY_URL = "http://localhost:3500/gateway/v1";
      ...
      const userDataDir = path.join(baseTmpDir, `Syrex-e2e-tests-${Date.now()}`);
      ...
      (electronApp as any).$SyrexUserDataDir = userDataDir;
      ...
      if (os.platform() === "win32") {
        try {
          console.log("[cleanup:start] Killing Syrex.exe");
          console.time("taskkill");
          execSync("taskkill /f /t /im Syrex.exe");
          console.timeEnd("taskkill");
          console.log("[cleanup:end] Killed Syrex.exe");
        } catch (error) {
          console.warn(
            "Failed to kill Syrex.exe: (continuing with test cleanup)",
            error,
          );
        }
      } else {
    ...
    .map((message) => {
      const content = Array.isArray(message.content)
        ...
        : message.content
            ...
            .replace(
              /\n<Syrex-file path="package\.json">[\s\S]*?<\/Syrex-file>\n/g,
              "",
            );
      return `===\nrole: ${message.role}\nmessage: ${content}`;
    })
